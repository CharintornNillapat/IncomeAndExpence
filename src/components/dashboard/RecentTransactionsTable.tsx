...
export const RecentTransactionsTable: React.FC<RecentTransactionsTableProps> = React.memo(({
  ...
  return (
    <div className="bg-white rounded-2xl border border-stone-200 p-4 sm:p-5 shadow-2xs space-y-3 sm:space-y-4">
      <div className="flex items-center justify-between gap-3 pb-3 sm:pb-4 border-b border-stone-100">
        <div className="flex items-center gap-2">
          <Calendar className="w-5 h-5 text-indigo-600 shrink-0" />
          <div>
            <h2 className="text-sm sm:text-base font-bold text-stone-900">Recent Transactions</h2>
            <p className="hidden sm:block text-xs text-stone-500">Your latest 5 financial activities</p>
          </div>
        </div>
        ...
        <motion.button
          ...
          className="min-h-[44px] inline-flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-semibold text-stone-700 hover:text-stone-900 bg-stone-100 hover:bg-stone-200 rounded-xl transition-all cursor-pointer"
        >
          <span>View All</span>
              ...
              transactions.map((tx) => {
                ...
                return (
                  <tr key={tx.id} className="hover:bg-stone-50/70 transition-colors">
                    <td className="py-3 px-3 sm:px-4 font-mono text-stone-500 whitespace-nowrap text-[11px] sm:text-xs">
                      {tx.transactionDate}
                    </td>

                    <td className="py-3 px-3 sm:px-4 font-medium text-stone-900 max-w-[170px] sm:max-w-[240px]">
                    ...
                    <td className="hidden md:table-cell py-3 px-4 text-stone-600 font-medium">
                    ...
                    <td className="py-3 px-3 sm:px-4 text-right font-mono font-bold whitespace-nowrap text-xs sm:text-sm">
                      ...
                      <span className="text-[10px] text-stone-400 ml-1 hidden sm:inline">{currency}</span>
                    </td>
                  </tr>
                );
              })
