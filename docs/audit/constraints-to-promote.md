...
| State persists through one batched writer; add a slice by registering it there, not by adding another effect | `FinanceContext.tsx` — `localStorage.setItem` down to 2 call sites (the writer + its flush) | Yes — re-verified Phase 37, still 2 sites | State | T16 | pending |
...
| Every interactive element added to a view carries an `id` following `<view>-<thing>-<kind>` | `AuthModal.tsx` has 8 `id=` attributes (was 0) | No — `AuthModal.tsx` itself still complies (re-verified Phase 37); still not re-verified as a repo-wide invariant across the other 12 views/components. Not a Phase 32-37 task, so not audited this pass either | Testing | T12 | |
