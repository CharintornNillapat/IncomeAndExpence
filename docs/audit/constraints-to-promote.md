...
| State persists through one batched writer; add a slice by registering it there, not by adding another effect | `FinanceContext.tsx` — `localStorage.setItem` down to 2 call sites (the writer + its flush) | Yes — re-verified Phase 37, still 2 sites | State | T16 | 24e9ee6 |
