...
**Roadmap status: closed out at Phase 29; Phase 30 is a separately-requested follow-on.** All 29 original phases (T1–T50) are `done`. Phase 29 (T50, 2026-09-19) was the closeout pass — ADRs 0006–0008, final metrics, and constraint promotion into `CLAUDE.md`. Phase 30 (T51–T56, 2026-09-19) landed after that closeout, at the user's explicit request: a duplicate-category correctness bug fix plus the "Categories & Smart Rules" hub feature build. Two items remain outside both closed passes by their own explicit design, not by oversight: `T25`/`T18` in the **Deferred** table below, each blocked on a separate approval this session never received. See that table's own notes for why each stayed out.
